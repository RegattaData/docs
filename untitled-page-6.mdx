...
- It is not allowed to include a vector distance expressions in a hierarchical `ORDER BY `with `LIMIT `using approximate search. For example, the following queries are not supported and would result in an error: <br />
  ```text
  ```