...
The data type of the returned value appears with the right arrow (⇒) sign.
...
1. `MOD()`: Arguments can be either `integral_type` (`INT`, `BIGINT`, `SMALLINT`) or `DECIMAL`. It
cannot be floating-point numbers (`FLOAT` or `REAL`)