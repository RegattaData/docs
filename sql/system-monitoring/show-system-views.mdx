...
| `show devices`       | `device_id`   | `null`   | `device_id`          | show all the devices of the database                  |
...
| `show metric description` | `metric_name` | `null`   | `metric_name`        | show all the database metrics and their description |
| `show module metrics`  | `metric_name` | `null`   | `metric_name,module_id` | show all the modules metrics and their values         |
| `show modules`       | `Module_role` | `Null`   | `Module_role,node_name` | show all the modules in the cluster                   |
| `show nodes`         | `node_name`   | `null`   | `node_name`          | show all the nodes in the cluster                     |
| `show system views`  | `view_name`   | `null`   | `view_name`          | show this view                                        |
| `show table devices` | `device_id`   | `TABLE`  | `device_id`          | show the devices of a specific table                  |
| `show tables`        | `table_name`  | `null`   | `table_name`         | show all the tables in the database                    |
| `show transactions`  | `null`        | `null`   | `start_time`         | show all the active transactions in the database       |
| `show users`         | `user_name`   | `null`   | `user_name`          | show all the users in the database                      |
