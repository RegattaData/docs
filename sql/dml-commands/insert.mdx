...
where device_list is a comman-separated list of device identifiers and module_list is a comma-separated list of the module identifiers of the modules to which the devices belong.
```
...
Regatta uses an internal algorithm to distribute the rows of a table across the table storage devices optimally. You can override it by specifying one or more device into which the rows will be inserted. The modules containing the devices should also be specified.
