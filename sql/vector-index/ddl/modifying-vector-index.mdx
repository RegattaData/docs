...
Dropped modules must be in final state, i.e. `COMPLETE `or `FAILED`. (See [Show Vector Index Modules](/sql/vector-index/show-commands) command for description of module states). Aborted modules must be in state `BUILDING`, i.e., during the build process. 
