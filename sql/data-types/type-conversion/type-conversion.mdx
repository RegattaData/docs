...
1. When passing to functions and operators, operands of different types than their input operand types.
1. When assigning values of different types to table row cells.
...
3. During `INSERT` and `UPDATE` operations, Regatta converts the value to the datatype of the affected column. If the value is too large for the target data type, the operation fails.
3. Conversion from `TIMESTAMP` to `DATE` or `TIME` extracts the relevant part of the `TIMESTAMP` into the target element.
...
6. Any conversion (explicit or implicit) to a numeric type (integers, floating point and decimals) would fail if the value cannot fit within the range of the target type.
...
9. `CHAR` types are implicitly converted to `VARCHAR` (and the trailing spaces are trimmed) when they are used as inputs to any function/operation.
   ...
   Note: If the `VARCHAR` contains trailing spaces as part of its string, the comparison will return false. For example: `CHAR(10`) with the text 'Regatta' `VARCHAR(10)` with the text 'Regatta '.