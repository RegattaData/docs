...
The shutdown process is performed asynchronously. The command returns when the system state has changed to `SHUTTING DOWN` .
...
<Icon icon="circle-exclamation" />  The specified failure thresholds replace the existing set of thresholds.
...
<Icon icon="circle-exclamation" />  See description of the failure thresholds in the [SYSTEM MODIFY](#system-modify) command.