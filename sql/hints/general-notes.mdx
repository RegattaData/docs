...
1. All hints should be given in comments with the ‘++’ sign:

    1.1. Failure to include the “++” right after the comment opening characters will result in regarding the hint as a regular comment, effectively ignoring it.
...
5. Multiple hints should be separated by commas: E.g.: `/*++ JOIN METHOD (HASH), INDEX (b, my_idx) */`
6. Note: A syntax error in a hint-comment may result in a parsing error that would cause ignoring all the hints in the comment, even if some hints are valid.
7. A hint must be applied by the query planner unless it is invalid. Invalid hints should be ignored. A hint is invalid if either of the following holds:
    ...
    7.1. It is impossible to apply the hint (e.g., a hint to use an index scan rather than a table scan when the relevant column is not indexed)
    
    7.2. Applying the hint would lead to incorrect results.
