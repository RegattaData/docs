...
| **regatta.mode**                   | string  | `INSERT`    | Sink connector mode:<br>• `CDC`: Handle Debezium-style change events (insert/update/delete).<br>• `INSERT`: Only insert new rows.<br>• `UPDATE`: Only update existing rows. |
...
| **regatta.auto.create**            | boolean | `false`     | Automatically create the target table if it does not exist.                                                                                                                 |
