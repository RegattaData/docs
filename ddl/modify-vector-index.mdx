...
Dropped modules must be in final state, i.e. `COMPLETE `or `FAILED`. (See [SHOW VECTOR INDEX MODULES](/vi-show-commands) command for description of module states). Aborted modules must be in state `BUILDING`, i.e., during the build process. 
