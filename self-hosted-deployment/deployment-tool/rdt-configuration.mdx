...
Downloading the Regatta tar archive can be done directly on one of the cluster nodes (using `wget` or a similar tool) or on an external host and then copied to one of the cluster nodes. The tar archive includes the RPM packages and the Regatta deployment tool.
...
- `internalIp` – The host’s internal IP address used for inter-node communication. This assumes you have an internal netwrok for all nodes in the cluster. If that is not the case, you can put an external IP here instead. 
