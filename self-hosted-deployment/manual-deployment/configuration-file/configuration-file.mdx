...
As stated, a configuration file example will be deployed for each module as part of the RPM installation. This file can be found at:

```code
/opt/regatta/1.0/<module name>/conf/cluster.template
```

The Regatta modules are expecting to read a cluster configuration file named `cluster.config`. You can create a `cluster.config` file from scratch, following a convetion explained in the coming section of this document, or use the `cluster.template` file as a base template. 
