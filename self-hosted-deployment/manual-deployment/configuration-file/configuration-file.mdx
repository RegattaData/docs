...
The Regatta modules are expecting to find a cluster configuration file named `cluster.config` under each module's conf directory. You can create a `cluster.config` file from scratch, following a convetion explained in the coming section of this document, or use the `cluster.template` file as a base template.
...
Those parameters are module-specific, and should be configured for each module individually.

The recommended way to handle this is to:\\

1. Create a common configuration file WITHOUT including the module-specific parameters at the end of the file and save it as `cluster.config`\\
2. Copy the common `cluster.config` to all the cluster's servers and put a copy of it in each module's `conf` directory.\\
3. Edit the `cluster.config` under each module conf folder to include the module specific parameters for that module.
...
The `cluster.config` file must be written following a few convention rules:

-  The config file must have a single "system" section.
- The config file must have at least 1 node section.
- The config file must have at lease one RDB module as well as one of each GDD, Sequencer and DCM.
- The config file must have at least one NVMe device for each RDB module.
- Each module must be linked to a node via the `parent_node_id` parameter
- Each NVMe device must be linked to its controlling rDB module via the `module_id` parameter
- Every node, module and device must have a unique id parameter, starting at **1**.
- Each node, module and device unique id is part of every parameter of that 
- The device `capacity` parameter is written in KB.