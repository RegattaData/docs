...
6. **RegattaDB Software:**<br />Obtain the RegattaDB software package and ensure it is available on all servers designated for the deployment
   <Note>
     If you are using the [**Regatta Deployment Tool (RDT)**](/self-hosted-deployment/rdt/introduction), the software package only needs to be available on the server where the RDT is executed. The RDT automatically distributes the required RPM packages to the remaining servers.
   </Note>
