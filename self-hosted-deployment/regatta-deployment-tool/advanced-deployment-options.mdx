...
Once the script execution with either the `config` or `update_config` commands is done, you can connect to any of the cluster nodes/servers via SSH to edit the cluster config file for any module. For further details, follow the steps in the [configuration](/self-hosted-deployment/regatta-deployment-tool/configuration) section and apply the desired changes.
