...
The deployment user must have read and write access to all storage devices used by RegattaDB.
...
During installation, the RPM packages automatically create a default deployment user and group named `regatta`. If you wish to use a different deployment user, follow these steps:
...
<Warning>
  When installing with `sudo`, the environment variables must be placed **after** `sudo`, not before it:

  ```bash
  # Correct - sudo passes the variables through to the installed command
  sudo REGATTA_USER=myuser REGATTA_GROUP=mygroup yum install regatta-rdb-<version>.el8.x86_64.rpm
  
  # Incorrect - sudo resets the environment by default, so these are
  # dropped before yum ever sees them, silently falling back to `regatta`
  REGATTA_USER=myuser REGATTA_GROUP=mygroup sudo yum install regatta-rdb-<version>.el8.x86_64.rpm
  ```
</Warning>
