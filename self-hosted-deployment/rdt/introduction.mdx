---
title: "Introduction"
description: "Easily automate RegattaDB deployment and management across multiple nodes."
---

<Note>
  This section assumes you're already familiar with the [RegattaDB software modules](/self-hosted-deployment/manual-deployment/regatta-software-modules) (SM, SNA, GDD, DCM, Sequencer, RDB) and basic deployment concepts such as a **node** (a physical or virtual machine in your deployment) and a **module** (a RegattaDB component running on a node).
</Note>
...
- **Connects to your nodes over SSH**, in parallel, so setting up a multi-node deployment
  doesn't mean repeating the same steps by hand on every node.
