...
Switch to the `deployment_user` mentioned in previous pages and run `setup_cluster.sh` (RDT) to begin the automated deployment:
...
The RDT will then validate SSH connectivity to every node defined in [config.json](https://docs.regatta.dev/self-hosted-deployment/scripted-solution/configjson). After successful validation, it will ask whether the `deployment_user` requires a password for sudo operations:
