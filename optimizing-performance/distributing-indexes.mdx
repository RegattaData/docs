...
The index may be given segments on multiple devices and distributed to multiple nodes to get more parallelism and performance.  If the index segments all contain roughly the same number of entries, this will also spread the load evenly, parallelizing scans optimally and making key lookups quickest on average.  On the other hand, having many small index segments can lead to excess operators.
...
<Note>Note: distribution helps parallelization of operations.  However, in the case of a small table, you should consider placing the data and index segments of the table on one and the same device.</Note>
...
>>>>>>> 2f5ad21 (Documentation edits made through Mintlify web editor)
